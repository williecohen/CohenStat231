...
Pitching <- Pitching %>%
  filter(total_wins > 299, total_SO > 2999)
...
> ANSWER: There does not appear to be a clear linear relationship, maybe a slight negative correlation, between number of inspections and the median score received by restaurants in these inspections. 

>In the visual representation, we see the majority of restaurants have relatively similar amounts of inspections (from 0 to around 100) and the majority of scores lie between 0 and 45.0. These individual median scores also do seem to resemble a sort of normal distribution (though the axes are flipped) centered around the score 15. It's important to note that though visually there may appear to be a slight right skew there are relatively few points past the score of 45 and thousands between the scores 0 and 45. 
...
g <- g +
      geom_curve(x = 1500, xend = 1840, y = 25, yend = 14, 
             arrow = arrow(length = unit(0.3, "cm")), curvature = -0.4, color = "black") + 
      geom_text(x = 1500, y = 35, label = "This is Subway\n(median score = 15)",color = "black")
...
FakeDataWide <- FakeDataLong %>%
  gather(key = measure, value = value, meanL, sdL, meanR, sdR) %>%
  mutate(measure2 = paste(as.character(sex), measure, sep = ".")) %>%
  select(-c(sex, measure)) %>%
  spread(key = measure2, value = value, fill = 0)
