...
> RESPONSE: According to the help file, the file has 1846 rows and 3 variables indicating that there are 3 columns.
...
> RESPONSE: This correlatoin coefficient implies that there is a weak, negative linear relationshup between x and y in the dino dataset.
...
> RESPONSE: There seems to be a realtionship between x and y in that they make a picture of a dinosaur which is not captured in the correlation coefficient as it is not a linear relationship.
...
> RESPONSE: This value of -0.06296 is very similar to the correlation coefficient from our dino dataset, which is -0.06447, indicating that both have a weak negative linear relationship.
...
> RESPONSE: Though the correlation coefficients were similar between the two data sets, the plots look very different. This second plot outlines a star as opposed to the dionsaur from the first plot. 
...
> RESPONSE: Again, we see a similar weak negative linear relationship evidenced in the correlation coefficient of -0.06834, closely resembling both the star and dino datasets. 
...
> RESPONSE: This plot looks different from the other 2 data sets in that it looks like an oval or circle compared to the dinosaur and star from the previous two, again showing that similarities in linear correlation do not indicate similarities in the data we examine across different datasets. 
...
> RESPONSE: Since the correlation coefficient denotes the strength of the linear relationship between the x and y variables, it isn't an appropriate summary for these data sets. So while all of the correlation coefficients are rather low, this does not necessarily indicate a lack of realtionship between the two variables, as clearly through the pictures there exists some sort of relationship in each data set. 
