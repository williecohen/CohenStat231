...
flightsJoined2 %>%
  ...
  summarize(N = n(), avg_arr_delay = (sum(arr_delay)/N)) %>%
...
> ANSWER: This data set is called "narrow" as it contains realtively few variables and thus has only a few columns but a large amount of rows or values for each of these variables. Since there are so many names indexed within the data set, this narrow presentation means that we cannot view that dataset altogether at once. 
...
> ANSWER: The three names with the largest ration are Justice, which has a ratio of 0.972 male to female, Skyler, which has a ratio of 0.773 male to female, and Quinn, which has a ratio of 0.763 female to male. 
...
> ANSWER: In our original BabyNarrow dataset, names which have no females or no males are omitted entirely, meaning that there is not a separate female and male listing for names that have no value for that sex. However, when we gather the BabyWide data back into BabyNarrow2, each of these names which had a zero for either M or F is assigned a completely new row. Thus, our observations for BabyNarrow2 are double those for BabyWide as each of the rows in BabyWide now becomes 2 rows in BabyNarrow2. Thus, we have more rows in our table but we are still representing the same data as the each of the new rows added has a "total" value of 0. 
